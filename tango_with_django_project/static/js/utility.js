...
$.ajaxSetup({ 
	...
	$('#upload-button .upload').click(function() {
		...
		var x = 0;
		var y = 0;
		var post_responsed = false;
		
		$('#loading-content').html('Uploading '+dataArray[0].name);
		
		$.each(dataArray, function(index, file) {	
			
			$.post('/rango/upload/', dataArray[index], function(data) {
				...
				var result = data['successful'];
				if(result) {
					var realData = '<li><a href="images/">'+fileName+'</a></li>';
					
					$('#uploaded-files').append('<li><a href="images/"></a> '+data.name+'</li>');
					...
					$('#uploaded-files').append('<li><a href="images/" File Name: '+data.name+'</li>');
					 $('#files').append(data.name);
                     $('#files').append(" ");
				}
	...
	if(post_responsed && window.localStorage.length > 0) {
